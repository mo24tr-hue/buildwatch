 ...
 * Full product workspace opens as a real company (Acme Builders) via onOpenWorkspace.
 */
export default function PlatformAdmin({ profile, session, onLogout, onOpenWorkspace }) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-2xl mx-auto px-4 py-5">
        <div className="mb-5 border border-black rounded-md p-4">
          <div className="font-medium">Acme Builders</div>
          <p className="text-sm text-[#6B6E72] mt-1">
            Full company workspace for walkthroughs and screenshots. Same app as contractors use — phases, photos, costs, change orders.
          </p>
          <button
            type="button"
            disabled={opening || !onOpenWorkspace}
            onClick={openWorkspace}
            className="mt-3 w-full py-2.5 rounded text-sm text-white bg-black disabled:opacity-50"
          >
            {opening ? 'Opening…' : 'Open Acme Builders'}
          </button>
        </div>
          ...
          <div>
            ...
            {filteredFeedback.length === 0 ? (
              <p className="text-sm text-[#6B6E72]">
                No feedback yet. Users send it from menu → Send feedback.
              </p>
            ) : (
              <div className="space-y-2">
